...
describe "BigDecimal#ceil" do
  ...
  it "returns the smallest integer greater or equal to self, if n is unspecified" do
    ...
    @zero_neg.ceil.should == @zero_pos   # Maglev deviation
  ...
  it "returns n digits right of the decimal point if given n > 0" do
    # @mixed.ceil(1).should == BigDecimal("1.3") # Maglev, error in spec, 
...
#   BigDecimal("0.03").ceil(1).should == BigDecimal("0.1") # Maglev, error in spec
...
#   BigDecimal("23.45").ceil(1).should == BigDecimal('23.5') # Maglev, error in spec
    BigDecimal("23.451").ceil(1).should == BigDecimal('23.5') #   per default ROUND_HALF_UP
...
#   BigDecimal("2E-10").ceil(9).should == BigDecimal('1E-9') 
    BigDecimal("2E-10").ceil(9).should == BigDecimal('0')  # Maglev deviation per rounding
...
#     @one.div(@three,20).ceil(n).should == BigDecimal("0.#{'3'*(n-1)}4")
      @one.div(@three,20).ceil(n).should == BigDecimal("0.#{'3'*(n-1)}3") # Maglev rounding
      # 1.4, 1.34, 1.334, etc.
