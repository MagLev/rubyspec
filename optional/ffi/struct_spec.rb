...
describe "Struct tests" do
  it "Struct#[:pointer]" do
    ...
    (ax = s[:pointer]).should == (bx = smp.get_pointer(0)) # make debugging easier
  ...
  it "Struct#[:pointer].nil? for NULL value" do
   ...
   deviates_on :maglev do
    s[:pointer]._equal?(nil).should == true
   end
  end
  ...
  it "Struct#[:pointer]=nil" do
   ...
   deviates_on :maglev do
    smp.get_pointer(0)._equal?(nil).should == true
   end
  end
  ...
  it "Struct#layout works with :name, :type, offset tuples" do
    alignll = FFI::TYPE_UINT64.alignment  # don't assume a 32bit CPU
    pair_layout = Class.new(FFI::Struct) do
    ...
    s[:b] = 0xfee1deadbeef
    mp.get_int64(alignb).should == 0xfee1deadbeef
  end
 ...
 not_compliant_on :maglev do # CallbackMember not supported yet
...
describe FFI::Struct, ' with an array field'  do
 ...
 not_compliant_on :maglev do  # does not cache
...
describe "Struct allocation" do
  ...
  it "MemoryPointer.new(Struct, 2)" do
   ...
   not_compliant_on :maglev do # can't construct a Struct from a Fixnum
