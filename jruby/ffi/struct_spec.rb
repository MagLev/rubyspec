...
describe "Struct aligns fields correctly" do
  it "char, followed by an int" do
    class CIStruct < FFI::Struct
      # layout :c => :char, :i => :int
      layout( :c , :char, :i , :int ) # Maglev, hash layout not supported
    end
  ...
  it "short, followed by an int" do
    class SIStruct < FFI::Struct
      ...
      layout( :s , :short, :i , :int) # Maglev, hash layout not supported
    end
  ...
  it "int, followed by an int" do
    class IIStruct < FFI::Struct
      # layout :i1 => :int, :i => :int
      layout( :i1 , :int, :i , :int) # Maglev, hash layout not supported
    end
  ...
  it "long long, followed by an int" do
    class LLIStruct < FFI::Struct
      ...
      layout(  :i , :int, :l , :long_long ) # Maglev, hash layout not supported , reverse order to pass
    end
...
describe "Struct tests" do
  ...
  it "Struct#[:pointer]" do
    ...
    (ax = s[:pointer]).should == mp  # maglev debugging
  end
  ...
  it "Struct#[:pointer]=" do
    ...
    s = PointerMember.new smp
    s[:pointer] = mp
    (ax = smp.get_pointer(0)).should == mp # maglev debugging
  end
  ...
  it "Struct#layout works with :name, :type, offset tuples" do
    class PairLayout < FFI::Struct
      alignll = FFI::TYPE_UINT64.alignment # maglev fix spec to work on 64bit cpu
    ...
    mp.get_int(0).should == 0x12345678
    s[:b] = 0xfee1deadbeef
    ofs = FFI::TYPE_UINT64.alignment == 4 ? 4 : 8  # maglev fix spec
    mp.get_int64(ofs).should == 0xfee1deadbeef
  end
  ...
  if rb_maj.to_i >= 1 && rb_min.to_i >= 9 || RUBY_PLATFORM =~ /java/
    it "Struct#layout withs with a hash of :name => type" do
      class HashLayout < FFI::Struct
        # layout :a => :int, :b => :long_long
        layout( :a , :int, :b , :long_long) # Maglev, hash layout not supported
      end
      ll_off = (FFI::TYPE_UINT64.alignment == 4 ? 4 : 8)
...
# it ":float field r/w" do	# maglev float not supported yet
#   klass = Class.new(FFI::Struct)
