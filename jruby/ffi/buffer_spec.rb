...
#       :float => 4, :double => 8
         :double => 8  # maglev , float not supported yet
    }.each_pair do |t, s|
...
describe "Buffer#put_pointer" do
  it "put_pointer(0, p).get_pointer(0) == p" do
    p = FFI::MemoryPointer.new :ulong_long  # maglev, fix spec, add FFI::
    p.put_uint(0, 0xdeadbeef)
    buf = Buffer.alloc_inout 8
    p2 = (bx = buf.put_pointer(0, p)).get_pointer(0)
    p2.should_not be_nil
#    p2.should == p			# maglev fails
#   p2.get_uint(0).should == 0xdeadbeef   			# maglev fails
  end
end
